...
class ServiceTransformer extends BumblebeeTransformer {
  ...
  transform (model) {
    return {
     ...
     value: model.value,
     initial_date: moment(model.initial_datetime).format("DD/MM/YYYY"),
     final_date: moment(model.final_datetime).format("DD/MM/YYYY"),
     initial_time: moment(model.initial_datetime).format("HH:mm"),
     final_time: moment(model.final_datetime).format("HH:mm"),
     status: model.status,
